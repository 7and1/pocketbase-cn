...
export async function GET(context: RSSContext) {
  ...
  const sortedPosts = posts.sort(
    (a: { data: { publishDate: { valueOf: () => number } } }, b: { data: { publishDate: { valueOf: () => number } } }) =>
  ...
  return rss({
    ...
    items: sortedPosts.map((post: { data: { title: string; description?: string; excerpt?: string; publishDate: Date; updatedDate?: Date; category?: string; author?: string }; slug: string }) => ({
